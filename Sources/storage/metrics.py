...
class StoredMetric(Storable):
	...
	def __init__( self, name, value, meta=None, timestamp=None ):
		if timestamp is None: timestamp = getTimestamp()
		self.name      = name
		self.value     = value
		self.meta      = meta
...
class MetricStorage:
	...
	def get( self, name, after=None, before=None ):
		...
		for _ in self.backend.get(name):
			...
			if after  is not None and not m.isAfter(after):   continue
			if before is not None and not m.isBefore(before): continue
			yield m
