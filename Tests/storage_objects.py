...
from storage_base import Attachment, Message, MemoryBackend, ObjectStorage, RawStorage, DirectoryBackend, A, B
import unittest, os, shutil, sys, json

class StoredObjectTest(unittest.TestCase):

	def setUp( self ):
		if hasattr(self,"objects"): self.objects.release()
		if hasattr(self,"raw"):     self.raw.release()
